...
User::User(string id, string n, string p, string c) : PLATFORM_NAME("PakWheels") {
    userId = id; name = n; phone = p; city = c; isVerified = true;
}
User::User(const User& other) : PLATFORM_NAME("PakWheels") {
    userId = other.userId; name = other.name; phone = other.phone; city = other.city; isVerified = other.isVerified;
}
void User::login() { cout << name << " logged in to " << PLATFORM_NAME << endl; }
void User::logout() { cout << name << " logged out." << endl; }
void User::viewProfile() const { cout << "User: " << name << " | City: " << city << endl; }
void User::updateProfile(string newCity) { city = newCity; cout << "Profile Updated." << endl; }